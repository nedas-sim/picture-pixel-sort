...
void PrintData(int ms, int size, string fileName)
{
...
void Replace(DataArray<int> arr, int height, int width)
{
...
void SwapRows(DataArray<int> arr, int a, int b, int width)
{
    for (int i = 0; i < width; i++)
        arr.Swap(a * width + i, b * width + i);
}

DataArray<int> Reshuffle(DataArray<int> arr, int height, int width)
{
...
void ArrowDown(int num, DataArray<int> array, int m, int n, ref int toWrite, DataArray<int> pixels)
{
    int x = num;
    int y = 0;
    if (num >= m)
...
void ArrowUp(int num, DataArray<int> array, int m, int n, ref int toWrite, DataArray<int> pixels)
{
    int x = m - num - 1;
    int y = n - 1;
    if (x < 0)
...
DataArray<int> LeftHalf(DataArray<int> array)
{
...
DataArray<int> RightHalf(DataArray<int> array)
{
    int size1 = array.Size() / 2;
...
void Merge(DataArray<int> result,
...
void MergeSort(DataArray<int> array)
{
    if (array.Size() > 1)
    {
